function TechnologyCard({ tech, isSelected, onAdd }) {
  return (
    <div className="border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition">
      ...
      <p className="text-sm text-gray-500 mb-4 leading-relaxed">{tech.description}</p>
      <div className="flex items-center gap-3 text-xs text-gray-500 mb-4">
        <span className="px-2 py-1 bg-gray-100 rounded-md">{tech.category}</span>
        <span className="px-2 py-1 bg-gray-100 rounded-md">{tech.difficulty}</span>
        <span className="ml-auto flex items-center gap-1 text-amber-500 font-medium">★ {tech.rating}</span>
      </div>
      <button
        onClick={() => onAdd(tech)}
        disabled={isSelected}
